...
def bas_rnd(num,base):
    #return base * round(num/base)
    return num-(num%base)

def diagonal_len(num):
    return int(num*math.sqrt(2))
...
def get_nearest_pol(lst,c_pos,tw):
    nearest_pol=(None,tw+1)
    for ind,pol in enumerate(lst):
        min_dist=None
        for tile in pol.tiles:
            dist=dist_calc(c_pos,tile.xy)
            if min_dist is None:
                min_dist=dist
            elif min_dist>dist:
                min_dist=dist
        if nearest_pol is (None,tw+1):
            nearest_pol=([ind],min_dist)
        elif nearest_pol[1]>min_dist:
            nearest_pol=([ind],min_dist)
        elif nearest_pol[1]==min_dist:
            nearest_pol[0].append(ind)
    return nearest_pol

def get_wall_pos(angle,x,y):
    return (0,round(math.tan(math.radians(angle))*-x)-y)

class Tile:
    def __init__(self, x, y, size, color=(253,2,45)):
        self.rect = pyg.Rect(x, y, size,size)
        self.id=int(f'{x}{y}')
        self.color = color

    def draw_edges(self, screen,col1=(240,240,240),col2=(240,240,240)):
        directions=[self.rect.topright,self.rect.topleft,self.rect.bottomleft,self.rect.bottomright]
        for ind,point in enumerate(directions):
            pyg.draw.circle(screen,col1,point,3)
            pyg.draw.line(screen,col2,point,directions[(ind+1)%4])

    def draw_hitbox(self, screen):
        pyg.draw.rect(screen, self.color, (self.rect.x, self.rect.y, self.rect.w, self.rect.h))
        #pyg.draw.polygon(screen, self.color, ((self.rect.x, self.rect.y), (self.rect.x+self.rect.width, self.rect.y), (self.rect.x+self.rect.width/2, self.rect.y+self.rect.height)))

class Cursor:
    def __init__(self):
        pyg.mouse.set_visible(False)
        self.rect = pyg.Rect(0, 0, 5, 5)
        
    def update(self,screen):
        mx, my = pyg.mouse.get_pos()
        self.rect.x = mx
        self.rect.y = my
        pyg.draw.circle(screen,(240,240,240),(self.rect.x,self.rect.y),5)
